...
const AuthLayout = ({ children }: AuthLayoutProps) => {
  ...
  return (
    <div className="w-full flex flex-col lg:flex-row flex-1 min-h-screen">
      {/* LEFT — illustration / branding (desktop only) */}
      <aside className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-[#FFF5F5] via-[#FAF7FF] to-[#F0EEFF] items-center justify-center p-14 xl:p-20 min-h-screen">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[420px] h-[420px] rounded-full bg-white/40 blur-2xl" />
        ...
        <div className="relative z-10 w-full max-w-[520px] flex flex-col items-center text-center">
          ...
          <p className="mt-4 poppins-regular text-[#5C5C5C] text-[15px] leading-relaxed max-w-[400px]">
          ...
          <div className="mt-10 w-full flex flex-col gap-3">
            {features.map(({ icon: Icon, label, description, accent }) => (
              <div
                key={label}
                className="flex items-start gap-4 rounded-2xl bg-white/80 backdrop-blur-sm border border-white/60 px-5 py-4 text-left shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all duration-300 hover:shadow-[0_12px_40px_rgba(158,42,43,0.08)] hover:-translate-y-0.5"
              >
                <div
                ...
                <div className="min-w-0 pt-0.5">
                  <p className="poppins-semibold text-[#1A1A1A] text-sm leading-none">
                    {label}
                  </p>
                  <p className="mt-1.5 poppins-regular text-[#6F6F6F] text-[13px] leading-snug">
                    {description}
                  </p>
                </div>
              </div>
      ...
      {/* RIGHT — form */}
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-white px-6 sm:px-10 py-10 sm:py-12 min-h-screen overflow-y-auto">
